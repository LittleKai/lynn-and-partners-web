...
export default function FiltersAndActions({
  ...
  const exportToCSV = () => {
    try {
      ...
      toast({
        title: t("csvSuccessTitle"),
        description: `${filteredProducts.length} ${t("csvSuccessDesc")}`,
      });
    } catch (error) {
      toast({
        title: t("exportFailTitle"),
        description: t("exportFailDesc"),
        variant: "destructive",
      });
    }
  };
  ...
  const exportToExcel = () => {
    try {
      ...
      toast({
        title: t("excelSuccessTitle"),
        description: `${filteredProducts.length} ${t("excelSuccessDesc")}`,
      });
    } catch (error) {
      toast({
        title: t("exportFailTitle"),
      ...
      <div className="flex justify-center">
        <div className="flex items-center gap-2 bg-muted p-2 rounded-lg">
          <span className="text-sm font-medium text-muted-foreground">
            {t("export")} {filteredProducts.length} {t("products")}:
          </span>
          <Button
            variant="outline"
            size="sm"
    ...
    <div className="flex flex-col sm:flex-row gap-3 poppins">
      {selectedStatuses.length > 0 && (
        <div className="border-dashed border rounded-sm p-1 flex gap-2 items-center px-2 text-sm">
          <span className="text-gray-600">{tColumns("status")}</span>
          ...
          <div className="flex gap-2 items-center">
            {selectedStatuses.length < 3 ? (
              ...
              <Badge variant={"secondary"}>{tFilters("selected3")}</Badge>
            )}
          </div>
        </div>
      )}
      ...
      {selectedCategories.length > 0 && (
        <div className="border-dashed border rounded-sm p-1 flex gap-2 items-center px-2 text-sm">
          <span className="text-gray-600">{tColumns("category")}</span>
          <Separator orientation="vertical" />
          <div className="flex gap-2 items-center">
      ...
      {selectedSuppliers.length > 0 && (
        <div className="border-dashed border rounded-sm p-1 flex gap-2 items-center px-2 text-sm">
          <span className="text-gray-600">{tColumns("supplier")}</span>
          <Separator orientation="vertical" />
          <div className="flex gap-2 items-center">
